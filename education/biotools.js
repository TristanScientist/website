...
function onDataLoaded(error, linkData) {
  ...
  divs.append("h4").text(function (d) {
    return d.heading;
  });
  ...
  var links = divs.append("div").attr("class", "newspaper"); // cascaded method call
  // now append an <a> tag for each link
  links
    .selectAll("a")
    .data(function (cat) {
      return cat.links;
    }) // bind to each nested <div>, the links property of the corresponding category
    .enter()
    .append("a")
...
d3.json = function (url, callback) {
  callback(false, JSON.parse(d3.select("pre").text()));
};
